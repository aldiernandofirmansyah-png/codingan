...
// Get stats
$total_event = mysqli_fetch_assoc(mysqli_query($connection, "SELECT COUNT(*) as total FROM events"))['total'];
$event_aktif = mysqli_fetch_assoc(mysqli_query($connection, "SELECT COUNT(*) as aktif FROM events WHERE status='aktif'"))['aktif'];
$event_draft = mysqli_fetch_assoc(mysqli_query($connection, "SELECT COUNT(*) as draft FROM events WHERE status='draft'"))['draft'];

// Get events
$events_query = mysqli_query($connection, "SELECT * FROM events ORDER BY created_at DESC");
?>
...
<body class="bg-light">
    ...
    <div class="row">
      ...
      <nav class="col-md-3 col-lg-2 d-md-block bg-dark sidebar text-white min-vh-100">
        <div class="text-center py-3 border-bottom">
          <img src="logoo.png" alt="Logo Kampus" width="160" class="img-fluid mb-2" />
        </div>
          ...
          <div class="col-md-4">
            <div class="card border-primary text-center">
              <div class="card-body">
                ...
                <h3 class="fw-bold text-primary"><?= $total_event ?></h3>
              </div>
            </div>
          </div>
          <div class="col-md-4">
            <div class="card border-success text-center">
              <div class="card-body">
                ...
                <h3 class="fw-bold text-success"><?= $event_aktif ?></h3>
              </div>
            </div>
          </div>
          <div class="col-md-4">
            <div class="card border-warning text-center">
              <div class="card-body">
                ...
                <h3 class="fw-bold text-warning"><?= $event_draft ?></h3>
              </div>
            </div>
          </div>
        </div>
              ...
              <table class="table table-striped align-middle text-center">
                ...
                <tbody>
                  <?php if(mysqli_num_rows($events_query) > 0): ?>
                    <?php $no = 1; while($row = mysqli_fetch_assoc($events_query)): ?>
                      <tr>
                        <td><?= $no++ ?></td>
                        <td>
                          <?php if(!empty($row['gambar'])): ?>
                            <img src="<?= $row['gambar'] ?>" alt="<?= htmlspecialchars($row['nama_event']) ?>" class="img-fluid rounded" width="80" style="object-fit: cover; height: 60px;">
                          <?php else: ?>
                        ...
                        <td>
                          <button class="btn btn-sm btn-warning me-1" data-bs-toggle="modal" data-bs-target="#editEventModal" 
                                  onclick="setEditData(<?= $row['id'] ?>, '<?= htmlspecialchars($row['nama_event'], ENT_QUOTES) ?>', '<?= htmlspecialchars($row['lokasi'], ENT_QUOTES) ?>', '<?= $row['status'] ?>', '<?= $row['tanggal_mulai'] ?>', '<?= $row['tanggal_selesai'] ?>', `<?= addslashes($row['deskripsi']) ?>`, '<?= $row['kategori'] ?>')">
                            <i class="bi bi-pencil"></i> Edit
                          </button>
                          <a href="hapus_event.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" 
                    ...
                    <?php endwhile; ?>
                  <?php else: ?>
                    <tr><td colspan="9" class="text-center text-muted py-4">Belum ada event yang ditambahkan.</td></tr>
                  <?php endif; ?>
                </tbody>
              </table>
            </div>
        ...
        <div class="modal-body">
          <form method="POST" action="tambah_event.php" enctype="multipart/form-data" id="formTambahEvent">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="gambar" class="form-label fw-bold">Gambar Event</label>
            ...
            <div class="mb-3">
              <label for="deskripsi" class="form-label fw-bold">Deskripsi Event <span class="text-danger">*</span></label>
              <textarea class="form-control" id="deskripsi" name="deskripsi" rows="4" placeholder="Deskripsikan event secara detail..." required></textarea>
            </div>
        ...
        <div class="modal-body">
          <form method="POST" action="edit_event.php" enctype="multipart/form-data" id="formEditEvent">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="edit_nama_event" class="form-label fw-bold">Nama Event <span class="text-danger">*</span></label>
                ...
                <div class="mb-3">
                  <label for="edit_lokasi" class="form-label fw-bold">Lokasi <span class="text-danger">*</span></label>
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="edit_gambar" class="form-label fw-bold">Gambar Event</label>
                ...
                <div class="mb-3">
                  <label for="edit_tanggal_mulai" class="form-label fw-bold">Tanggal Mulai <span class="text-danger">*</span></label>
                  <input type="date" class="form-control" id="edit_tanggal_mulai" name="tanggal_mulai" required>
                </div>
                <div class="mb-3">
                  <label for="edit_tanggal_selesai" class="form-label fw-bold">Tanggal Selesai <span class="text-danger">*</span></label>
                  <input type="date" class="form-control" id="edit_tanggal_selesai" name="tanggal_selesai" required>
                </div>
              </div>
            </div>
            <div class="mb-3">
              <label for="edit_deskripsi" class="form-label fw-bold">Deskripsi Event <span class="text-danger">*</span></label>
              <textarea class="form-control" id="edit_deskripsi" name="deskripsi" rows="4" placeholder="Deskripsikan event secara detail..." required></textarea>
            </div>
            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
              <button type="button" class="btn btn-secondary me-md-2" data-bs-dismiss="modal">Batal</button>
  ...
  <script>
    ...
    function setEditData(id, nama, lokasi, status, tanggal_mulai, tanggal_selesai, deskripsi, kategori) {
